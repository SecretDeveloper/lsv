...
mod internal_tests
    ...
    fn sort_and_reselect_by_name()
    {
        ...
        let idx_b =
        (0..100).find(|&i| app.get_current_entry_name(i).as_deref() == Some("b.txt"));
        if let Some(i) = idx_b
        {
            app.select_index(i);
        }
...
mod input_tests
    ...
    fn navigation_and_parent_current_dir_changes()
    {
        ...
        if app.current_has_entries()
        {
            ...
            // Find index of 'sub'
            if let Some(idx) =
            (0..100).find(|&i| app.get_current_entry_name(i).as_deref() == Some("sub"))
            {
...
mod app_rs_tests
    ...
    fn refresh_preview_trims_to_preview_lines()
    {
        ...
        if let Some(pos) =
        (0..100).find(|&i| app.get_current_entry_name(i).as_deref() == Some("long.txt"))
        {
