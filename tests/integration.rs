...
"#;
        ...
        let (cfg, maps, engine_opt) = lsv::config::load_config_from_code(code, Some(std::path::Path::new("."))).expect("load config");

        assert_eq!(cfg.config_version, 1);
        assert_eq!(cfg.keys.sequence_timeout_ms, 600);
        assert_eq!(cfg.ui.show_hidden, true);
        assert_eq!(cfg.ui.preview_lines, 80);
        assert_eq!(cfg.ui.max_list_items, 1234);
        assert_eq!(cfg.ui.panes.as_ref().map(|p| (p.parent, p.current, p.preview)), Some((10, 20, 70)));
        assert_eq!(cfg.ui.date_format.as_deref(), Some("%Y"));
        assert_eq!(cfg.ui.row.as_ref().map(|r| r.icon.as_str()), Some("X "));
        assert_eq!(cfg.ui.row_widths.as_ref().map(|w| (w.icon, w.left, w.middle, w.right)), Some((2, 40, 0, 12)));
        assert_eq!(cfg.ui.theme.as_ref().and_then(|t| t.item_fg.as_deref()), Some("white"));
        assert_eq!(cfg.ui.theme.as_ref().and_then(|t| t.dir_fg.as_deref()), Some("blue"));
        ...
        let mut by_seq: std::collections::HashMap<String, String> = std::collections::HashMap::new();
        for m in &maps { by_seq.insert(m.sequence.clone(), m.action.clone()); }
        assert_eq!(by_seq.get("q").map(String::as_str), Some("quit"));
        assert!(by_seq.get("ss").map(|s| s.starts_with("run_lua:")).unwrap_or(false));
        assert!(by_seq.get("gs").map(|s| s.starts_with("run_lua:")).unwrap_or(false));

        let action_count = engine_opt.as_ref().map(|(_, _, keys)| keys.len()).unwrap_or(0);
...
"#;
        let (cfg, _maps, _eng) = lsv::config::load_config_from_code(code, Some(&root)).expect("load config");
        assert_eq!(cfg.ui.date_format.as_deref(), Some("%Y"));
    }
    ...
    fn restricted_require_blocks_parent_traversal() {
        ...
        let err = lsv::config::load_config_from_code(bad_code, Some(&root)).err().expect("should error");
        let msg = format!("{}", err);
        assert!(msg.contains("invalid module name") || msg.contains("inline init.lua execution failed"));
    }
}
...
"#;
        let (_cfg, _maps, engine_opt) = lsv::config::load_config_from_code(code, None).expect("load with action");
        ...
        let (fx, overlay) = lsv::actions::lua_glue::call_lua_action(&mut app, 0).expect("call action");
        assert!(fx.quit);
    ...
    fn parse_effects_from_table() {
        ...
        assert!(fx.redraw);
        assert!(fx.quit);
        assert!(matches!(fx.messages, lsv::actions::effects::OverlayToggle::Toggle));
        assert!(matches!(fx.output_overlay, lsv::actions::effects::OverlayToggle::Show));
    ...
    fn parse_effects_defaults_when_missing() {
        ...
        assert!(matches!(fx.output_overlay, lsv::actions::effects::OverlayToggle::None));
    }
...
mod keymap_tests {
    ...
    fn keymap_prefix_building_and_lookup() {
        let mut app = lsv::app::App::new().expect("app new");
        let maps = vec![
            lsv::config::KeyMapping { sequence: "a".into(), action: "internal:noop".into(), description: Some("A".into()) },
            lsv::config::KeyMapping { sequence: "ab".into(), action: "internal:noop2".into(), description: Some("AB".into()) },
        ];
        app.test_set_keymaps(maps);
        assert_eq!(app.test_resolve_action("a").as_deref(), Some("internal:noop"));
        assert_eq!(app.test_resolve_action("ab").as_deref(), Some("internal:noop2"));
...
mod apply_tests {
    ...
    fn apply_config_overlay_relist_on_show_hidden() {
        let mut app = lsv::app::App::new().expect("app new");
        let lua = mlua::Lua::new();
        let tbl = lsv::config_data::to_lua_config_table(&lua, &app).expect("to table");
        ...
        let data = lsv::config_data::from_lua_config_table(tbl).expect("from table");
    ...
    fn apply_config_overlay_redraw_only_on_date_format() {
        ...
        assert!(app.test_force_full_redraw(), "date format change should force redraw");
    }
    ...
    fn apply_config_overlay_refresh_preview_only_on_preview_lines() {
        ...
        assert!(!app.test_force_full_redraw(), "preview-only changes should not force full redraw");
    }
...
mod dispatcher_tests {
    ...
    fn dispatch_internal_quit() {
        let mut app = lsv::app::App::new().expect("app new");
        let ran = lsv::actions::dispatch_action(&mut app, "quit").expect("dispatch");
        assert!(ran);
...
"#;
        ...
        let mut app = lsv::app::App::new().expect("app new");
        app.inject_lua_engine_for_tests(engine, keys);
        let ran = lsv::actions::dispatch_action(&mut app, "run_lua:0").expect("dispatch lua");
        assert!(ran);
        assert!(app.test_should_quit());
    }
...
"#;
        ...
        let ran = lsv::actions::dispatch_action(&mut app, "run_lua:0;sort:reverse:toggle").expect("dispatch seq");
        assert!(ran);
        assert!(app.test_should_quit());
    ...
    fn dispatch_unknown_action_returns_false() {
        let mut app = lsv::app::App::new().expect("app new");
        let ran = lsv::actions::dispatch_action(&mut app, "no_such_action").expect("dispatch");
        assert!(!ran);
        assert!(!app.test_should_quit());
    }
}
...
mod defaults_actions_tests {
    ...
    fn zf_sets_friendly_display_mode() {
        ...
        let action = app.test_resolve_action("zf").expect("default zf mapping present");
        let ran = lsv::actions::dispatch_action(&mut app, &action).expect("dispatch zf");
        assert!(ran);
    ...
    fn zc_sets_info_created_and_does_not_crash() {
        let mut app = lsv::app::App::new().expect("app new");
        let action = app.test_resolve_action("zc").expect("default zc mapping present");
        let ran = lsv::actions::dispatch_action(&mut app, &action).expect("dispatch zc");
        assert!(ran);
        assert!(matches!(app.test_info_mode(), lsv::app::InfoMode::Created));
        // No render test here; just verifying setting InfoMode::Created does not panic
    }
}
...
mod internal_tests {
    ...
    fn sort_and_reselect_by_name() {
        ...
        let idx_b = (0..100).find(|&i| app.test_entry_name(i).as_deref() == Some("b.txt"));
        if let Some(i) = idx_b { app.test_select_index(i); }
        // Sort by size via internal action
        let ran = lsv::actions::dispatch_action(&mut app, "sort:size").expect("dispatch");
        assert!(ran);
    ...
    fn toggle_sort_reverse() {
        let mut app = lsv::app::App::new().expect("app new");
        let ran = lsv::actions::dispatch_action(&mut app, "sort:reverse:toggle").expect("dispatch");
        assert!(ran);
    ...
    fn set_info_and_display_modes() {
        ...
        assert!(lsv::actions::dispatch_action(&mut app, "display:friendly").unwrap());
        assert!(matches!(app.test_display_mode(), lsv::app::DisplayMode::Friendly));
    }
    ...
    fn navigation_top_bottom() {
        ...
        let mut app = lsv::app::App::new().expect("app new");
        app.test_set_cwd(dir);
        let ran = lsv::actions::dispatch_action(&mut app, "nav:bottom").expect("dispatch");
        assert!(ran);
        if app.test_has_entries() {
            let last = (0..).take(100).position(|i| app.test_get_entry(i).is_none()).unwrap_or(0).saturating_sub(1);
...
"#;
        let (_cfg, maps, engine_opt) = lsv::config::load_config_from_code(code, None).expect("load config");
...
lsv.set_previewer(function(ctx) return nil end)
"#;
        let (_cfg, _maps, engine_opt) = lsv::config::load_config_from_code(code, None).expect("load config");
...
"#;
        let (cfg, _maps, _eng) = lsv::config::load_config_from_code(code, None).expect("load config");
    ...
    fn defaults_are_loaded_when_no_user_code() {
        let (cfg, _maps, _eng) = lsv::config::load_config_from_code("", None).expect("load defaults");
...
"#;
    ...
    fn require_absolute_path_is_blocked() {
        ...
        let err = lsv::config::load_config_from_code(code, Some(&root)).err().expect("should error");
        let msg = format!("{}", err);
        assert!(msg.contains("invalid module name") || msg.contains("inline init.lua execution failed"));
    }
...
"#;
        let (_cfg, maps, _eng) = lsv::config::load_config_from_code(code, None).expect("load config");
        // Extract indices for our sequences
        let idx = |seq: &str| -> usize { maps.iter().find(|m| m.sequence == seq).and_then(|m| m.action.strip_prefix("run_lua:")?.parse().ok()).unwrap() };
        let i1 = idx("a1"); let i2 = idx("a2"); let i3 = idx("a3");
        assert!(i1 < i2 && i2 < i3);
    }
...
"#;
        ...
        assert!(maps.iter().any(|m| m.sequence == "y" && m.action.starts_with("run_lua:")));
    }
...
"#;
        ...
        assert_eq!(cfg.keys.sequence_timeout_ms, 0);
        assert_eq!(cfg.ui.preview_lines, 100);
        assert_eq!(cfg.ui.row_widths.as_ref().map(|w| (w.icon, w.left, w.middle, w.right)), Some((0, 0, 0, 0)));
        assert_eq!(cfg.ui.theme.as_ref().and_then(|t| t.item_fg.as_deref()), Some("123"));
    }

    #[test]
    fn set_previewer_wrong_type_errors() {
        let code = r#" lsv.set_previewer(123) "#;
        let err = lsv::config::load_config_from_code(code, None).err().expect("should error");
        let msg = format!("{}", err);
        assert!(msg.contains("lsv api install failed") || msg.contains("execution failed") || msg.contains("error"));
    }
...
mod config_data_tests {
    ...
    fn roundtrip_to_from_lua_table() {
        ...
        let cfgd = lsv::config_data::from_lua_config_table(tbl).expect("from table");
        ...
        assert_eq!(matches!(cfgd.ui.display_mode, lsv::app::DisplayMode::Friendly), true);
        ...
        assert_eq!(cfgd.ui.row_widths.as_ref().map(|w| (w.icon, w.left, w.middle, w.right)), Some((2, 40, 0, 12)));
        assert_eq!(cfgd.ui.theme.as_ref().and_then(|t| t.dir_fg.as_deref()), Some("cyan"));
        assert_eq!(cfgd.ui.theme.as_ref().and_then(|t| t.item_fg.as_deref()), Some("white"));
...
mod input_tests {
    ...
    fn key(ch: char) -> KeyEvent { KeyEvent::new(KeyCode::Char(ch), KeyModifiers::NONE) }
    ...
    fn sequence_timeout_clears_pending() {
        let mut app = lsv::app::App::new().expect("app new");
        app.test_set_keymaps(vec![lsv::config::KeyMapping { sequence: "xy".into(), action: "quit".into(), description: None }]);
        // short timeout
...
mod app_rs_tests {
    ...
    fn refresh_preview_trims_to_preview_lines() {
        ...
        if let Some(pos) = (0..100).find(|&i| app.test_entry_name(i).as_deref() == Some("long.txt")) {
...
mod runtime_rs_tests {
    ...
    fn process_event_quit_returns_true() {
        let mut app = lsv::app::App::new().expect("app new");
        let quit = lsv::runtime_util::process_event(&mut app, Event::Key(KeyEvent::new(KeyCode::Char('q'), KeyModifiers::NONE))).unwrap();
        assert!(quit);
    }
    ...
    fn process_event_resize_returns_false() {
        let mut app = lsv::app::App::new().expect("app new");
        let cont = lsv::runtime_util::process_event(&mut app, Event::Resize(80, 24)).unwrap();
        assert!(!cont);
    }
}
...
mod partial_return_tests {
    ...
    fn partial_overlay_return_is_merged_safely() {
        // Action returns only a tiny table; validator should still see a full config
        let code = r#"
...
mod lua_glue_tests {
    ...
    fn make_app_with_actions(lua_src: &str, seq: &str) -> (lsv::app::App, usize) {
        let (_cfg, maps, engine_opt) = lsv::config::load_config_from_code(lua_src, None).expect("load lua");
...
"#;
        ...
        let pos = (0..100).find(|&i| app.test_entry_name(i).as_deref() == Some("hello.txt")).expect("find hello");
        app.test_select_index(pos);
        let (fx, _ov) = lsv::actions::lua_glue::call_lua_action(&mut app, idx).expect("call");
...
"#;
        ...
        if app.test_has_entries() {
            ...
            let last = (0..100).position(|i| app.test_get_entry(i).is_none()).unwrap_or(0).saturating_sub(1);
            assert_eq!(app.test_selected_index(), Some(last));
        }
    }
...
"#;
        ...
        let (fx, ov) = lsv::actions::lua_glue::call_lua_action(&mut app, idx).expect("call");
        // Effects may be empty; overlay should exist because we mutated config
        if let Some(data) = ov { lsv::actions::apply::apply_config_overlay(&mut app, &data); }
        assert!(matches!(app.test_display_mode(), lsv::app::DisplayMode::Friendly));
...
"#;
        ...
        let (_fx, ov) = lsv::actions::lua_glue::call_lua_action(&mut app, idx).expect("call");
