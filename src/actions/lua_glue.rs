...
pub fn call_lua_action(app: &mut App, idx: usize) -> io::Result<(ActionEffects, Option<crate::config_data::ConfigData>)> {
  ...
  if idx >= funcs.len() { return Ok((ActionEffects::default(), None)); }
...
fn build_lsv_helpers(lua: &Lua, cfg_tbl: &Table, app: &App) -> io::Result<Table> {
  ...
  let select_item_fn = lua.create_function(move |lua, idx: i64| {
    let ctx: Table = match cfg_ref.get("context") { Ok(t) => t, Err(_) => {
  ...
  let os_run_fn = lua.create_function(move |_, cmd: String| {
    ...
    match out {
      Ok(output) => {
        ...
        trace::log(format!("[os_run] exit={:?} bytes_out={}", output.status.code(), buf.len()));
        Ok(true)
      }
      Err(e) => {
...
//   output_title=`$ cmd`, and logs timing/exit info. Env provided:
//   LSV_PATH, LSV_DIR, LSV_NAME (derived from current selection).
// - lsv.os_run_interactive(cmd): suspends TUI (leave alt screen + disable
//   raw mode), runs attached to terminal, then restores TUI and requests
//   a full redraw. On non-zero exit, it writes a short note to Output.
