...
//
// - `apply_effects` handles transient UI state (selection, overlays,
//   output, redraw, quit).
...
pub fn apply_effects(app: &mut App, fx: ActionEffects) {
  ...
  match fx.messages {
    OverlayToggle::Toggle => { app.show_messages = !app.show_messages; app.show_output = false; app.show_whichkey = false; }
    OverlayToggle::Show => { app.show_messages = true; app.show_output = false; app.show_whichkey = false; }
    OverlayToggle::Hide | OverlayToggle::None => {}
  }

  match fx.output_overlay {
    OverlayToggle::Toggle => { app.show_output = !app.show_output; app.show_messages = false; app.show_whichkey = false; }
    OverlayToggle::Show => { app.show_output = true; app.show_messages = false; app.show_whichkey = false; }
    OverlayToggle::Hide | OverlayToggle::None => {}
  }
...
pub fn apply_config_overlay(app: &mut App, data: &crate::config_data::ConfigData) {
  ...
  let current_panes = app
    .config
    .ui
    .panes
    .clone()
    .unwrap_or(crate::config::UiPanes { parent: 30, current: 40, preview: 30 });
