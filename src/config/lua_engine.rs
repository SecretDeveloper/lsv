...
impl LuaEngine
{
    ...
    pub fn new() -> LuaResult<Self>
    {
        ...
        // Inject `lsv` namespace with stub APIs that accept calls from user
        // config.
        {
            let globals = lua.globals();
            let lsv: Table = lua.create_table()?;

            // lsv.config(tbl): accept and store later (currently a no-op
            // returning true)
            let config_fn =
                lua.create_function(|_, _tbl: mlua::Value| Ok(true))?;

            // lsv.mapkey(seq, action, description?): accept and store later
