        ...
        .create_function(move |lua, tbl: Value| {
            if let Value::Table(t) = tbl
            {
                ...
                if let Ok(icons_tbl) = t.get::<Table>("icons")
                {
                    ...
                    // Legacy: icons.by_ext (deprecated). Parse first so
                    // `extensions` wins.
                    ...
                    if let Ok(f_tbl) = icons_tbl.get::<Table>("by_name")
                    {
                        for pair in f_tbl.pairs::<Value, Value>()
                        {
                            ...
                            if let (Value::String(ks), Value::String(vs)) =
                                (k, v)
                                && let (Ok(k), Ok(v)) =
                                    (ks.to_str(), vs.to_str())
                            {
                                icons
                                    .folders
                                    .insert(k.to_lowercase(), v.to_string());
                            }
                        }
                    }
                ...
                // Top-level actions table (collect both Lua fn and string
                // actions)
                if let Ok(actions_tbl) = t.get::<Table>("actions")
                {
                    for pair in actions_tbl.sequence_values::<Value>()
                    {
                        if let Value::Table(t) = pair.map_err(|e| {
                            LuaError::RuntimeError(e.to_string())
                        })?
                        {
                            // Lua function action: fn = function(lsv, config)
                            // ... end
                            if let Ok(func) = t.get::<mlua::Function>("fn")
                            {
                                let keymap =
                                    t.get::<String>("keymap").map_err(|e| {
                                        LuaError::RuntimeError(e.to_string())
                                    })?;
                ...
                match keymaps_val
                {
                    Value::String(s) =>
                    {
                        let seq = s
                            ...
                            .map(|v| v.to_string())
                            .unwrap_or_default();
                        maps_for_actions_outer.borrow_mut().push(
                            super::KeyMapping {
                                sequence:    seq,
        ...
        match val
        {
            ...
            Value::String(s) =>
            {
                ...
                if let Some(root) = theme_root
                {
                    ...
                    let path =
                        root.join("lua").join(format!("{}.lua", rel_path));
