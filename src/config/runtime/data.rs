...
) -> mlua::Result<Table>
{
    ...
    let sel_idx =
        app.list_state.selected().map(|i| i as u64).unwrap_or(u64::MAX);
...
pub fn from_lua_config_table(tbl: Table) -> Result<ConfigData, String>
{
    ...
    if let Ok(ui) = tbl.get::<Table>("ui")
    {
        ...
        if let Ok(s) = ui.get::<String>("display_mode")
        {
            ...
            {
                return Err("ui.display_mode must be one of: \
                            absolute|friendly"
                    .to_string());
            };
    ...
    if let Ok(ui) = tbl.get::<Table>("ui")
    {
        if let Ok(v) = ui.get::<String>("sort")
        {
            ...
            {
                return Err("sort.key must be one of: name|size|mtime|created"
                    .to_string());
            };
            data.sort_key = k;
        }
