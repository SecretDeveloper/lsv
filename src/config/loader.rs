        ...
        let key = match key_opt
        {
            ...
            None =>
            {
                let f: mlua::Function = lua
                    .create_function(|_, _ctx: mlua::Value| {
                        Ok(mlua::Value::Nil)
                    })
                    .map_err(|e| {
                        io_err(format!("create noop previewer failed: {e}"))
                    })?;
                lua.create_registry_value(f).map_err(|e| {
                    io_err(format!("registry noop previewer failed: {e}"))
                })?
            }
        };
...
) -> io::Result<ConfigArtifacts>
{
    ...
    let base = match config_root.as_ref()
    {
        Some(p) => p.clone(),
